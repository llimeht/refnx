...
\end{itemize} 
Each of these points are frequently neglected. For example, the use of different software versions may change the output from an analysis. The use of a GUI program may not record the full set of steps, or options, applied by a user.
...
The usefulness of open-source software in a git repository must be emphasised here - the ability to return to a specific software version/environment is often difficult with proprietary or closed source software; different versions can have modifications that can significantly change the output of an analysis. This has important implications for verifiability.
